...
import os
from ml_tools.utils.metrics import ClassificationMetrics
import torch

class TrainerMultiClassClassification(BaseTrainer):
    ...
    def train_one_epoch(self, epoch):
        self.network.train()
        loss = 0.0
        for _, (imgs, labels) in enumerate(self.trainloader):
    ...
    def validate_one_epoch(self, epoch):
        ...
        self.network.eval()
        with torch.no_grad():
            for _, (imgs, labels) in enumerate(self.valloader):
                labels = labels.type(torch.LongTensor) 
                imgs, labels = imgs.to(self.device), labels.to(self.device)
                preds = self.network(imgs)
                loss = self.criterion(preds, labels)
